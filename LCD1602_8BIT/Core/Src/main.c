...
void LCD_Write_Byte(uint8_t mode, uint8_t data)
{
	if(mode == 0x00)
	{
		//data
...
void LCD_Init()
{
	...
	HAL_Delay(50);
	LCD_Write_Cmd(0x38); // che do hien thi 
	HAL_Delay(5);
	LCD_Write_Cmd(0x0C); //Xoa tro chuot
	HAL_Delay(5);
	LCD_Write_Cmd(0x01); // clear man hinh
	HAL_Delay(5);
	LCD_Write_Cmd(0x06);//che do viet chu
	HAL_Delay(5);
...
void LCD_SetCursor(uint8_t row, uint8_t col)
{
	...
	{
		adr = 0x80 +col;
		LCD_Write_Cmd(adr);// gui lenh di chuyen con tro
...
int main(void)
{
  ...
  MX_GPIO_Init();
  /* USER CODE BEGIN 2 */
	HAL_GPIO_WritePin(GPIOB, GPIO_PIN_10, 0); //E = 0
	LCD_Init();
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
		LCD_SetCursor(0,0);
		LCD_Write_String("Xin Chao");
		LCD_SetCursor(1,0);
		LCD_Write_String("Toi la Nam");
		HAL_Delay(3000);
		LCD_Write_Cmd(0x01); // clear man hinh
		HAL_Delay(5);
		LCD_SetCursor(0,0);
		LCD_Write_String("Toi 19 tuoi");
		LCD_SetCursor(1,0);
		LCD_Write_String("Toi o Ha Noi");
		HAL_Delay(3000);
		LCD_Write_Cmd(0x01); // clear man hinh
		HAL_Delay(5);
		LCD_SetCursor(0,0);
		LCD_Write_String("Rat vui khi gap ban");
		LCD_SetCursor(1,0);
		LCD_Write_String("Hen gap lai sau");
		HAL_Delay(3000);
		LCD_Write_Cmd(0x01); // clear man hinh
		HAL_Delay(5);
